...
def resolve_execution_provider_keys(model_context : str) -> List[ExecutionProviderKey]:
	if has_execution_provider('coreml') and (model_context.startswith('facefusion.processors.modules.age_modifier.styleganex_age') or model_context.startswith('facefusion.processors.modules.frame_colorizer')):
		return [ 'cpu' ]
